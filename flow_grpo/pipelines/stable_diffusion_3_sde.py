...
__all__ = [
    ...
    "sd3_compute_log_prob"
]


def sd3_compute_log_prob(
    prev_sample: torch.Tensor, prev_sample_mean: torch.Tensor, std_dev_t: torch.Tensor, dt: torch.Tensor) -> torch.Tensor:
    log_prob = (
...
class StableDiffusionPipelineOutputeWithSDELogProb(BaseOutput):
    ...
    all_log_probs: List[Union[torch.Tensor, None]]
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                output = self.scheduler.step(noise_pred, t, latents, noise_level=noise_level, return_dict=True)

                if isinstance(output, FlowMatchEulerSDEDiscreteSchedulerOutput):
                    log_prob = sd3_compute_log_prob(
                        output.prev_sample,
